...
export function DepartDetailsModal({ departId, isOpen, onClose }: DepartDetailsModalProps) {
  ...
  const renderBaseDetails = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
      <div className="space-y-2">
        ...
        <DetailItem label="Date création" value={new Date(depart.dateCreation).toLocaleDateString()} />
        <DetailItem label="Tension" value={`${depart.tension} V`} />
        <DetailItem label="Longueur totale" value={`${depart.longueurTotale} km`} />
      </div>
      <div className="space-y-2">
        ...
        <DetailItem label="Zones couvertes" value={
          <div>
            <div>Régions: {depart.zonesGeographiques.regions.join(', ')}</div>
            <div>Départements: {depart.zonesGeographiques.departements.join(', ')}</div>
            <div>Communes: {depart.zonesGeographiques.communes.join(', ')}</div>
            <div>Quartiers: {depart.zonesGeographiques.quartiers.join(', ')}</div>
          </div>
        } />
      </div>
    </div>
  ...
  const renderPosteOrigine = () => {
    if (!posteOrigine || posteOrigine.type !== 'POSTE_DISTRIBUTION') return null;
      ...
      <div className="mb-6">
        ...
        <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <DetailItem label="Tension" value={`${posteOrigine.niveauTension} V`} />
            <DetailItem label="Localisation" value={`${posteOrigine.quartier}, ${posteOrigine.commune}`} />
            <DetailItem label="État" value={posteOrigine.etatVisuel} />
          </div>
        </div>
      </div>
    );
    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <StatCard 
          ...
          description={Object.entries(stats.parType).map(([type, count]) => (
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-100 dark:bg-gray-700">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300">Désignation</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300">Localisation</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300">État</th>
            </tr>
          </thead>
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {actifsAssocies.map(actif => (
              <tr key={actif.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{actif.id}</td>
                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                  {actif.type.replace(/_/g, ' ')}
                </td>
                <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{actif.designationGenerale}</td>
        ...
        <div className="flex justify-center items-center mb-5">
          <div className="flex flex-col items-center w-full">
            ...
            <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-4 h-80 w-full">
              {/* Carte Leaflet */}
              {/* Carte Leaflet */}
{posteOrigine && posteOrigine.geolocalisation?.latitude && posteOrigine.geolocalisation?.longitude && actifsAssocies.length > 0 ? (
  <div className="h-full w-full">
    <MapContainer
      center={[posteOrigine.geolocalisation.latitude, posteOrigine.geolocalisation.longitude]}
    ...
    >
      ...
      <Marker 
        position={[posteOrigine.geolocalisation.latitude, posteOrigine.geolocalisation.longitude]}
        // Vous pouvez ajouter une icône personnalisée ici
      >
        <Popup>
          <div>
            <strong>Poste d'origine</strong><br />
            {posteOrigine.libelleCompte}<br />
            ID: {posteOrigine.id}
          </div>
        </Popup>
      </Marker>
      ...
      {actifsAssocies.map((actif, index) => {
        ...
        if (!lat || !lng || isNaN(lat) || isNaN(lng)) {
          console.warn(`Actif ${actif.id} a des coordonnées invalides:`, { lat, lng });
          return null;
        }
        ...
        return (
          ...
          >
            <Popup>
              <div>
                <strong>{actif.designationGenerale}</strong><br />
                Type: {actif.type.replace(/_/g, ' ')}<br />
                ID: {actif.id}<br />
      ...
      {(() => {
        ...
        const actifsAvecCoordonnees = actifsAssocies.filter(actif => {
          const lat = actif.geolocalisation?.latitude;
          const lng = actif.geolocalisation?.longitude;
          return lat && lng && !isNaN(lat) && !isNaN(lng);
        });
        
        console.log(`Actifs avec coordonnées valides: ${actifsAvecCoordonnees.length}/${actifsAssocies.length}`);
        
        if (actifsAvecCoordonnees.length < 2) {
          console.warn('Pas assez d\'actifs avec coordonnées pour tracer une ligne');
          return null;
        }
        
        // Créer les positions pour la polyline
        const positions = actifsAvecCoordonnees.map(actif => [
      ...
      {(() => {
        ...
        if (!firstActif) {
          console.warn('Aucun premier actif trouvé avec coordonnées valides');
          return null;
        }
        
        const connectionLine = [
          [posteOrigine.geolocalisation.latitude, posteOrigine.geolocalisation.longitude],
          [firstActif.geolocalisation.latitude, firstActif.geolocalisation.longitude]
        ] as [number, number][];
  ...
  <div className="flex items-center justify-center h-full">
    <div className="text-center">
      ...
      <p className="text-sm text-gray-400 dark:text-gray-500">
        Poste: {posteOrigine ? 'OK' : 'Manquant'} | 
        Actifs: {actifsAssocies.length} | 
        Avec coordonnées: {actifsAssocies.filter(a => a.geolocalisation?.latitude && a.geolocalisation?.longitude).length}
      </p>
    </div>
  </div>
)}
...
function StatusBadge({ status, label, large }: { status?: string; label?: string; large?: boolean }) {
  ...
  return (
    <span className={`${large ? 'px-4 py-2 text-sm' : 'px-3 py-1 text-xs'} rounded-full font-medium ${getStatusClass()}`}>
      {label ? `${label}: ` : ''}{status}
    </span>
  );
}

function StatCard({ title, value, description }: { title: string; value: React.ReactNode; description?: React.ReactNode }) {
  return (
    <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
